...
runners.map( word => firstNamesAllCaps.push(word.first_name.toUpperCase()));
...
runnersLargeSizeShirt =  runners.filter( word => word.shirt_size == "L");
console.log(JSON.stringify(runnersLargeSizeShirt));
...
ticketPriceTotal =  runners.reduce((count,curentV) => count += curentV.donation
,0);
...
// Problem 1
// all the runners add 50 more to there donation
  let newdonation = runners.map( ele => ele.donation+50)
  console.log(newdonation);



// Problem 2
//see how many XL shirt size is there
XL_size = 0;
runners.forEach(ele => {
  if (ele.shirt_size == 'XL'){
    XL_size++;
  }
} )
console.log(XL_size);
